...
export default class ResetPwPage extends HTMLElement {
	...
	render() {
			...
			<div id='auth-card-children' class="flex flex-col gap-y-2 w-full">
				<div class="flex flex-col gap-y-4 items-start w-full">
					...
					${this.state.emailEntered && !this.state.codeVerified ? `
						...
						<form id="verify-code" method="post" action="${serverURL}/auth-code/verify" class="flex flex-col gap-y-2 w-full">
					...
					${this.state.emailEntered && this.state.codeVerified ? `
						...
						<form id="reset-pw-form" method="post" action="${serverURL}/auth/reset" class="flex flex-col gap-y-2 w-full">
		...
		resetPwForm?.addEventListener('submit', async (e) => {
			...
			const action = new URL(resetPwForm.action)
		...
		emailForm?.addEventListener('submit', async (e) => {
			...
			const resCheckData = await fetch(`${serverURL}/auth/check`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json'},
		...
		verifyCode?.addEventListener('submit', async(e) => {
			...
			const action = new URL(verifyCode.action)
